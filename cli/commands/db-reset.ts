...
export default class DbResetCommand extends Command {
  async run() {
    ...
    if (
      !(await this.ui.confirm(
        `Are you sure you want to reset the ${config.DB_BACKEND} database? This will destroy all data in the current database!`
      ))
    ) {
    ...
    switch (config.DB_BACKEND) {
      ...
      default:
        this.ui.console.info(
          `The backend "${config.DB_BACKEND}" cannot be reset.`
        );
        process.exit(1);
    }
  }
  ...
  async destroyLocalDatabase(): Promise<boolean> {
    ...
    const dbLocalPath = config.DB_LOCAL_PATH;

    if (!dbLocalPath || dbLocalPath === '/' || dbLocalPath === '~') {
      this.ui.console.error(
        `❌ Invalid DB_LOCAL_PATH (${dbLocalPath}). Cannot reset database.`
      );
      return false;
    }
    ...
    if (!fs.existsSync(path.join(dbLocalPath, `${config.DB_NAME}.db`))) {
      this.ui.console.error(
        `⚠️ Database file "${config.DB_NAME}.db" does not exist in "${dbLocalPath}". Nothing to reset.`
      );
      return false;
    }
...
`);
    ...
    if (!config.DB_REMOTE_URL) {
      this.ui.console.error('❌ DB_REMOTE_URL not found in .env file');
      return false;
    }
    ...
    const spinner = this.ui
      ...
      .start();
    const serviceReady = await waitForService(config.DB_REMOTE_URL);
    if (!serviceReady) {
      spinner.stop();
    ...
    const url = `${config.DB_REMOTE_URL}/_users`;
