  ...
  async run() {
    ...
    if (config.DB_BACKEND !== 'remote') {
      this.ui.console.info(
        `"${config.DB_BACKEND}" database cannot be restored using the CLI.`
      );
      return;
    }
...
If you are not using a CouchDb database (DB_BACKEND is 'local'), then you just have to restore the database directory.
